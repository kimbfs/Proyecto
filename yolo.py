...
from common.utils import get_classes, get_anchors, draw_boxes, optimize_tf_gpu
...
optimize_tf_gpu(tf, K)
 ...
 #using Neural Compute Stick2 (NCS2)
#ncs2_plugin = IEPlugin(device="MYRIAD")
#model = IENetwork(model="/home/pi/iVision/weights/tinyyolov3_model.xml", weights="/home/pi/iVision/weights/tinyyolov3_model.bin")
#net= ncs2_plugin.load(network=model)
#print("Load in NCS2")
#inputBlob = next(iter(model.inputs))
#n = next(iter(model.outputs))

#n1, c, h, w = net.inputs[inputBlob].shape
...
class YOLO_np(object):
    ...
    def _generate_model(self):
        ...
        try:
            if self.model_type.startswith('yolo3_') or self.model_type.startswith('tiny_yolo3_'):
    ...
    def detect_image(self, image):
        ...
        image_array, salida = draw_boxes(image_array, out_boxes, out_classes, out_scores, self.class_names)
        ...
        inicio=time.time()
        out1=["The picture contains "]
        out2=np.concatenate((out1,salida))
        out=" "
        out= out.join(out2)
        print("Out:", out)
        tts=gTTS(out)
        tts.save('result.mp3')
        inicio2=time.time()
        #print(inicio2-inicio)
        call(['vlc','result.mp3','--play-and-exit'])
        time.sleep(1)
        fin=time.time()
        #print(fin-inicio2)
...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        #draw result on input image
        image_array = np.array(image, dtype='uint8')
        image_array = draw_boxes(image_array, out_boxes, out_classes, out_scores, self.class_names)
...
def detect_foto(yolo):
    while True:
        ...
        cam.release()
        image = Image.open("/home/pi/iVision/picture.jpg")
        r_image, _, _, _ = yolo.detect_image(image)
        r_image.show()
